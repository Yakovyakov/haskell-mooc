...
joinToLength n a = [x ++ y | x <- a, y <- a, length (x ++ y) == n]
...
multiCompose [] id = id
multiCompose (f:fs) id = (f . multiCompose fs) id
...
interpreter commands = helper (0, 0) commands
  where
    ...
    helper _ [] = []
    helper (x, y) (cmd:cmds) = case cmd of
      "up"     -> helper (x, y + 1) cmds
      "down"   -> helper (x, y - 1) cmds
      "left"   -> helper (x - 1, y) cmds
      "right"  -> helper (x + 1, y) cmds
      "printX" -> show x : helper (x, y) cmds
      "printY" -> show y : helper (x, y) cmds
      _        -> helper (x, y) cmds
