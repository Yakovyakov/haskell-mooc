...
data State = Start | EggsAdded | FlourAdded | SugarAdded | AllIngredients | Mixed | Error | Finished
  deriving (Eq,Show)
...
step FlourAdded AddSugar = AllIngredients
step FlourAdded _ = Error
step SugarAdded AddFlour = AllIngredients
step SugarAdded _ = Error
step AllIngredients Mix = Mixed
step AllIngredients _ = Error
...
instance Ord a => Semigroup (Set a) where
  Set xs <> Set ys = Set (mergeUnion xs ys)
    where
...
passwordAllowed password = checkRequirement
  where
    checkRequirement :: PasswordRequirement -> Bool
    checkRequirement (MinimumLength n) = length password >= n
    checkRequirement (ContainsSome s) = any (`elem` s) password
    checkRequirement (DoesNotContain s) = all (`notElem` s) password
    checkRequirement (Or r1 r2) = checkRequirement r1 || checkRequirement r2
    checkRequirement (And r1 r2) = checkRequirement r1 && checkRequirement r2
...
data Arithmetic = Literal Integer | Operation Arithmetic String Arithmetic
  deriving Show
...
operation op a b = Operation a op b

evaluate :: Arithmetic -> Integer
evaluate (Literal n) = n
evaluate (Operation a op b) = case op of
                            "+" -> evaluate a + evaluate b
                            "*" -> evaluate a * evaluate b

render :: Arithmetic -> String
render (Literal n) = show n
render (Operation a op b) = 
