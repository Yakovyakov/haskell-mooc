...
appendAll ref xs = mapM_ (\s -> modifyIORef ref (++ s)) xs
------------------------------------------------------------------------------
...
swapIORefs refA refB = do
    aVal <- readIORef refA
    bVal <- readIORef refB
    writeIORef refA bVal
    writeIORef refB aVal
...
hSelectLines h nums = do
  resultLines <- hFetchLines h
  return (map (resultLines !!) [i - 1 | i <- nums, i > 0 && i <= length resultLines])
...
interact' f state = loop state
  where
    loop currentState = do
        line <- getLine
        let (continue, output, newState) = f (line, currentState)
        putStrLn output
        if continue
          then loop newState
