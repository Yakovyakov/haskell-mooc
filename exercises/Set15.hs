...
calculator opStr numStr = 
  liftA2 ($) 
    (negate <$ check "negate" <|> (*2) <$ check "double") 
    (readMaybe numStr)
  where
    check name = if opStr == name then Just () else Nothing
...
validateDiv a b = if (b == 0) then invalid ("Division by zero!") else pure (div a b)
...
validateAddress streetName streetNumber postCode = Address <$> checkedAddress <*> checkedStreetNumber <*> checkedPostCode
  where checkedAddress = check (length streetName <= 20) "Invalid street name" streetName
...
--  boolOrInt "Falseb"  ==> Errors ["Not a Bool",]

boolOrInt :: String -> Validation (Either Bool Int)
boolOrInt s = 
  (Left <$> validateBool ) <|> (Right <$> validateInt )
  where
    validateBool = case readMaybe s of
      Just b  -> pure b
      Nothing -> invalid ("Not a Bool")
    
    validateInt = case readMaybe s of
      Just i  -> pure i
...
normalizePhone s = 
  let stripped = filter (/= ' ') s
      lengthCheck = check (length stripped <= 10) "Too long" stripped
      digitCheck = traverse validateChar stripped
      validateChar c = check (isDigit c) ("Invalid character: " ++ [c]) c
  in lengthCheck *> digitCheck
...
parseExpression s = 
  case words s of
    [arg1Str, opStr, arg2Str] -> 
      let
        opVal = validateOp opStr
        arg1Val = parseArg arg1Str
        arg2Val = parseArg arg2Str
      in opVal <*> arg1Val <*> arg2Val
...
checkNumber s = case readMaybe s of
      Just i  -> pure (Number i)
      Nothing -> invalid ("Invalid number: " ++ s)
checkVariable s = check ((length s == 1) && (isAlpha $ head s)) ("Invalid variable: " ++ s) (Variable (head s))
parseArg s = checkNumber s <|> checkVariable s
