...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node a leftTree rightTree  ) = 1 + (treeSize leftTree) + (treeSize rightTree)
...
treeMax Empty = 0
treeMax (Node a leftTree rightTree ) = maximum [a, treeMax leftTree, treeMax rightTree]
...
allValues condition (Node a rightTree leftTree) = condition a && allValues condition rightTree && allValues condition leftTree
------------------------------------------------------------------------------
...
walk (curr:steps) (Node a l r)
  | curr == StepL = walk steps l
  | otherwise = walk steps r
...
set (curr:path) val (Node a l r)
  | curr == StepL = (Node a (set path val l) r)
  | otherwise = (Node a l(set path val r))
