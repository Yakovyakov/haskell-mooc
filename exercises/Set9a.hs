...
countValid = length . filter valid
  where
    valid banknote =
      let len = length banknote
      in (len >= 5 && (banknote !! 2 == banknote !! 4)) || (len >= 6 && (banknote !! 3 == banknote !! 5))
...
lock :: Lock -> Lock
lock (Closed a) = (Closed a)
lock (Open a) = Closed a

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode _ (Closed a) = (Closed a)
changeCode pass _ = Open pass
...
permute perm input = map (input !!) inv
  where
    inv = [i | j <- (identity (length perm)),
                      let i = head [k | (k, p) <- zip [0..] perm, p == j]]