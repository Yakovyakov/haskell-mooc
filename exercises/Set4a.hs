...
allEqual [] = True
allEqual (x:xs) = foldr (\y acc -> y == x && acc) True xs
...
distinct xs = if length (nub xs) == length xs then True else False
...
longest = foldl1 (\x y ->
  ...
  else if length x < length y then y
  else if head x < head y     then x
  else y) 
...
-- incrementKey _ [] = []
-- incrementKey k (a:xs) = if k == fst a then (k, snd a + 1):incrementKey k xs else a:incrementKey k xs
-- 
-- List comprehensions
-- incrementKey key pairs = [ if k == key then (k, v + 1) else (k, v) | (k, v) <- pairs]
...
-- Challenge 1: Map.alter
-- freqs [] = Map.fromList []
-- freqs (x:xs) = Map.alter go x (freqs xs)
--   where
...
freqs = foldr (\x acc -> Map.alter (maybe (Just 1) (Just . (+1))) x acc) Map.empty
------------------------------------------------------------------------------
...
transfer from to amount bank
  | from `Map.notMember` bank = bank
  | to `Map.notMember` bank = bank
  | amount < 0 = bank
...
-- Data.Array.indices
-- maxIndex arr = foldr (\acc i -> if arr ! i > arr ! acc then i else acc) (head (indices arr)) (tail (indices arr))
