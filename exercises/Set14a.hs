...
greetText s
  | T.length s <= 15 = T.concat [(T.pack "Hello, "), s, (T.pack "!")]
  | otherwise = T.concat [(T.pack "Hello, "), (T.take 15 s), (T.pack "...!")]
...
shout text = 
  let wordsList = T.words text
      processedWords = map (\(i, word) -> if even i then T.toUpper word else word) 
                         (zip [0..] wordsList)
  in T.unwords processedWords
------------------------------------------------------------------------------
...
longestRepeat text
  ...
  | otherwise = maxRun
  where
    (_, _, maxRun) = T.foldl' step (T.head text, 1, 1) (T.tail text)
    step (prevChar, currentRun, maxRun) char
      | prevChar == char = 
          let newRun = currentRun + 1
              newMax = max newRun maxRun
          in (char, newRun, newMax)
      | otherwise = (char, 1, maxRun)
...
byteRange bs
  ...
  where
    (minVal, maxVal) = B.foldl' step (initial, initial) bs
    initial = B.head bs
    step (currentMin, currentMax) byte =
...
pingpong bs = BL.cycle (BL.fromStrict bs <> BL.fromStrict (B.reverse bs))
