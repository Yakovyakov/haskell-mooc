...
-- count x = foldl' (\acc y -> if x == y then acc + 1 else acc) 0
count x = getSum . foldMap (\y -> Sum (if x == y then 1 else 0))
------------------------------------------------------------------------------
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode x y rest) = f x (f y (foldr f acc rest))
