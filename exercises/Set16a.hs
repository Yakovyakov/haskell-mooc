...
sumIsLength input output = length input === (sum [y | (_, y) <- output])
...
genList :: Gen [Int]
genList = do
  lenghtOfList <- choose (3, 5)
  elementsForList <- (vectorOf (lenghtOfList) (choose (0, 10)))
  return (sort (elementsForList))
------------------------------------------------------------------------------
