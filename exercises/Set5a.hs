...
totalPrice (MkShoppingEntry desc price count )= price * fromIntegral count
...
buyOneMore (MkShoppingEntry desc price count )= (MkShoppingEntry desc price (count + 1))
...
data UpDown = UpMode { value :: Int} | DownMode { value :: Int}
  deriving (Show,Eq)
...
tick :: UpDown -> UpDown
tick (UpMode value) = UpMode {value = value + 1}
tick (DownMode value) = DownMode {value = value - 1}

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpMode value) = DownMode {value = value}
toggle (DownMode value) = UpMode {value = value}
...
fromBin b = go b 0 0
  where
    go End _ sum = sum
    go (O b) n sum = go b (n + 1) sum
    go (I b) n sum = go b (n + 1) (sum + 2^n)
...
toBin n
  ...
  | n == 0 = (O End)
  | n == 1 = (I End)
  | otherwise = if n `mod` 2 == 0 then O (toBin (n `div` 2)) else I (toBin (n `div` 2))

-- toBin' function that uses inc 
-- use inc :: Bin -> Bin
toBin' :: Int -> Bin
toBin' n = go (O End) n
  where
